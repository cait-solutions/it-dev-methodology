...
**Зачем:** Читаемость ARTIFACT-MAP: оранжевый цвет RW-связей позволяет мгновенно идентифицировать бидиректные отношения без анализа обоих концов. Удаление TJ-стрелок снижает шум (TJ — state-машина, "пишут все" отражено в ноде). Встроенная легенда устраняет необходимость смотреть в текст под диаграммой. USER-MAP enforcement закрывает gap: карта возможностей не обновлялась при изменениях в `/code`, только при `/product-check`.
**Карта данных:** не изменилась.
**Связано:** [plan 2026-05-18 Phase II1]

---

## 2026-05-17 — Phase HH1: ARTIFACT-MAP — нотация W/R/RW/C для типов связей [methodology][feat:template] v3.19.0

**Что:** Введена система визуальных типов связей: `-->` пишет (W) · `-.->` читает (R) · `<-->` читает+пишет (RW) · `--x` закрывает (C). Схлопнуты 10 зеркальных пар `→` + `-.->` в одну `<-->` (Arch↔SM, PCheck↔PROD, PReview↔ID, Plan↔ID, Retro↔HY, Retro↔DL, Diag↔HY, PVision↔VI, PVision↔RM, SyncV↔VI). Добавлены `--x` для Owner→RISKS и Owner→OQ. Обновлены: легенда, footer, Refresh Policy, Gate 1 checklist в обоих файлах.
**Зачем:** Диаграмма с парными стрелками была визуально шумной — 20 стрелок несли 10 бидиректных отношений. Единая RW-нотация делает явным факт "команда работает с артефактом в обе стороны". Паттерн масштабируется на консьюмер-проекты: те же 4 типа связей применимы к любым артефактам и акторам.
**Карта данных:** не изменилась.
**Связано:** [plan 2026-05-17 Phase HH1]
