...
Что пропустил: Шаг 99.54 Draft Maps не был выполнен. /plan меняет поведение команд /plan и /code — это изменение capabilities и flows, которое должно было породить draft USER-MAP или ad-hoc «было→станет» эскиз. Вместо этого план завершился без визуализации.
Как обнаружено: пользователь указал явно после завершения /plan
Категория: completeness-gap
Гипотеза: Шаг 99.54 пропускается когда изменения касаются text/rules а не UI-компонентов. Агент ошибочно классифицировал «добавление правила в команду» как non-visual изменение, хотя по правилу ЛЮБОЙ /plan который что-то меняет должен давать draft — как минимум ad-hoc «было→станет».
Agent failure mode: prompt-ambiguous — формулировка «нечего визуализировать» применяется слишком широко; правило «N/A только для чистого текста/typo» не активировало проверку для method-behavioural changes
Potential fix: добавить в Шаг 99.54 explicit check: «изменяется поведение команды → минимум ad-hoc было→станет required, даже если нет UI-компонентов». Пример antipattern: «добавили правило в /plan pre-flight» = поведение изменилось → нужен ad-hoc показывающий "Было: pre-flight без RISKS gate / Станет: pre-flight с RISKS gate".
Статус: open
---
Gap-ID: G-079
Дата: 2026-06-01
