...
**Consumer-facing changes:**
- **`model-tiers.md § Pre-flight model check` Шаг 3 переработан** — теперь две ветки по направлению mismatch:
  - **Under-powered** (текущий tier НИЖЕ рекомендованного, **любая ступень вниз**) → 🛑 **громкий русскоязычный STOP-advisory + пауза**: агент печатает фиксированный канонический блок (заголовок `# 🛑 СТОП — МОДЕЛЬ СЛАБЕЕ, ЧЕМ НУЖНО` + разделители) и **ждёт** `«продолжай»`/`«стоп»`. Раньше under-power ловился только при ≥2 ступени и тихо.
  - **Over-powered** (≥2 ступени вверх) → ⚪ одна строка FYI про экономию, **без паузы**.
  - Совпадение / 1 ступень вверх → тихо.
- **Канонический шаблон сообщения** живёт в `model-tiers.md` — команды на него ссылаются, не дублируют формат.
- **8 команд** (`code, onboard, plan, retro, review, roadmap, test, vision`) — строка Pre-flight model check переведена с устаревшего «≥ 2 ступени» на ссылку к канону (under-power порог изменился). `product-check` не тронут (там про over-power).

**Зачем:** под-power несёт quality risk (модель может упустить архитектурное нарушение) — асимметрично дороже cost waste, поэтому ловится даже на 1 ступень и громко. Поведение = advisory-пауза (не программный hard-block — UserPromptSubmit hook отложен до evidence по skip-rate, Ось 1). Сообщение всегда на русском, формат фиксирован для узнаваемости.

**Actions (при sync):**
1. `model-tiers.md` + 8 команд обновятся автоматически.
...
**Consumer-facing changes:**
- **`/plan` Шаг 1.8** (`[security]`/`[infra]` задачи) — переименован в «Threat model + User Journey», добавлен обязательный подблок **User Journey check**: путь живого человека (A) · самый болезненный шаг (B) · single-action happy path (C). VISION-anchor: оптимизация под один критерий ценой лишних шагов пользователя = нарушение Оси 1 + Actor-burden rule. Кормит Pre-Mortem #2.
- **`/opinion` мандат советника «Ценность»** — расширен вопросом про **user effort cost** (сколько шагов/действий до результата, есть ли путь в одно действие). Советник теперь явно флагует решения оптимизированные под безопасность/корректность ценой friction.

**Зачем:** закрывает класс «оптимизация под одну ось (threat-model) без учёта UX запуска» — инцидент с системой секретов (оптимизирована под threat-model, но ввод секрета потребовал сложной цепочки кликов). Решение через усиление существующих шагов (L3 input-structure), НЕ через новый советник или keyword-trigger (отвергнуто council 7/7 2026-06-22: дублирование + false-positive flood + нарушение Границы 8).

**Actions:** автоматически — ничего (аддитивно, graceful; приходит при следующем sync).
...
**Consumer-facing changes:**
- **`skills/secrets-management/SKILL.md`** — § File-type secrets переписан в **императивный agent-behavioral блок** (closes command-first violation из G-120). Для `type:file` секретов (GCP/Vertex JSON, сертификаты) **агент выполняет добавление САМ**: Edit manifest (`type:file`) → `set-secret.sh KEY .gcp/x.json` (путь не секрет → агент пишет) → `git check-ignore` → `validate-secrets.sh`; юзеру остаётся единственный шаг — положить JSON в `.gcp/`. **Hard-guard:** только для file-секретов; value-секрет (токен/key) агент НИКОГДА не пишет сам (значение утекло бы в transcript) — остаётся user-run. skill 1.1.0→1.2.0.
- **`commands/secrets.md`** — `--add`: для file-секретов агент делает сам (указатель на skill); value — user-run.
- **`scripts/validate-secrets.sh`** + **`templates/scripts/validate-secrets.sh`** (ADR-014 parity) — **BUGFIX:** `_parse_manifest` использовал tab-разделитель, а `read IFS=$'\t'` схлопывает пустые поля (tab = whitespace-IFS) → при пустых `service_name`/`service_url` (обычный случай для GCP file-секрета) поля сдвигались, `type` терялся → **проверка существования файла `type:file` молча не срабатывала** (G-120 verification была hollow когда метаданные пусты). Фикс: разделитель `\037` (unit separator, не whitespace) — пустые поля больше не схлопываются. Побочно чинит и v2-warnings (rotation/expiry/how_to_obtain читали неверные поля при пустых метаданных).
