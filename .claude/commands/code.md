...
**Pre-flight model check:** **да — при старте команды** определи текущую модель и сравни с tier из `/plan` рекомендации. Если mismatch ≥ 2 ступени — пауза + рекомендация перед началом реализации.
