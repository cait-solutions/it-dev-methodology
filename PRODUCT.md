...
|---|---|---|
| **Solo developer** (например PAI) | Один человек ведёт проект через Claude Code; нужны guard-rails чтобы не дрейфовать с архитектурой | Дисциплина "записать в DEVLOG", "проверить риски", "не лечить симптом" — методология автоматизирует через slash-команды и счётчики |
| **Team lead на multi-service** (например ERP) | Координирует AI-агентов нескольких разработчиков; нужна общая methodology чтобы PR от агентов соответствовали стандартам | Per-service триггеры в `/architecture-audit`, `/sync-vision` для разрешения конфликтов между vision и кодом, `/retro` для анализа skip-rates |
...
|---|---|---|
| Количество консьюмер-проектов | 2 (PAI, ERP) | 3+ к концу 2026 |
| Среднее расхождение версий между методологией и консьюмерами | n/a | < 1 minor bump |
| % команд имеющих single-source-of-truth (нет дрейфа PAI ↔ ERP) | 100% (после Phase A-F) | 100% |
