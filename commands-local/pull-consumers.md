...
```bash
# 1. Verify GITHUB_PAT (or analog for the host) is set:
bash scripts/check-secret.sh GITHUB_PAT  # exit 0 = present

# 2. Set if missing (one-time, per-developer):
bash scripts/set-secret.sh GITHUB_PAT <value>
