...
**Draft артефакты (если задача меняет акторов / flows / UX / компоненты):**

Для каждого из трёх артефактов — сгенерировать Mermaid-код и получить ссылку на Mermaid Live:

```
py scripts/mermaid-link.py --code "<mermaid-код>"
```
...
- 🗺 **USER-MAP `[DRAFT]`** — акторы + основные flows (≤ 5)
...
Если задача не меняет акторов/flows/компоненты → пропустить draft, указать явно почему.
