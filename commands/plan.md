...
⛔ **Заполнить все 7 строк таблицы обязательно.** Неприменимые → `N/A — [причина]`. Пропуск строки (не написать её вообще) = Шаг 99.3 не зачтён.
...
3. **Масштабируемое** — нагрузка + multi-use + расширяемость
   - ✅ `sync-methodology.sh` — независимая обработка файлов (выдержит 10× размер templates/) + reusable для consumers и самой методологии (multi-use) + легко расширяется новыми типами артефактов через templates/
   - ❌ Hardcoded path в одном скрипте — нельзя переиспользовать в другой команде без копи-пасты; добавление нового типа артефакта требует переписать функцию
   - 📝 N/A — для documentation change / methodology rule update / prompt-правила (нет runtime-нагрузки, multi-use неприменимо)
