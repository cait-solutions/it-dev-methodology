...
**Draft артефакты — scope-driven:**

Генерировать draft если задача меняет любое из:
- акторов / flows / UX / компоненты
- **структуру артефактов-карт** (USER-MAP / SYSTEM-MAP / ARTIFACT-MAP — добавление/удаление секций, переименование nodes, изменение связей)
...
Если задача не попадает ни под одно из условий выше → пропустить draft, указать явно почему.
