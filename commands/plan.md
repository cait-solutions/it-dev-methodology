...
Создать inline в ответе `/plan` (не файл — файл создаётся в `/code`):

- **USER-MAP `[DRAFT]`** — акторы + основные flows (≤ 5), Mermaid
- **SYSTEM-MAP `[DRAFT]`** — компоненты + их связи затронутые задачей, Mermaid
- **ARTIFACT-MAP `[DRAFT]`** — какие артефакты читаются/пишутся новыми командами/flows, Mermaid

Каждый draft помечается: `> ⚠️ [DRAFT — подтвердить scope перед /code]`

Показать пользователю и **явно спросить подтверждение** перед переходом к `/code`.
Если задача не меняет акторов/flows/компоненты → пропустить draft, указать явно почему.

`/code` Шаг 5 финализирует подтверждённые drafts: убирает маркер `[DRAFT]`, сохраняет/обновляет файлы.
