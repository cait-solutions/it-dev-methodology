...
7б. **ARTIFACT-MAP arrow type spot-check** (если есть `docs/product/ARTIFACT-MAP.md`):
   Для каждого `===` (RW) ребра в диаграмме — открой соответствующую команду и проверь:
   - Команда действительно И читает И пишет этот артефакт в одном запуске?
   - Если только читает → стрелка должна быть `-.->` (R), не `===`
   Для каждого `-->` (W) ребра — команда действительно пишет без чтения?
   - Если читает перед записью → возможно `===` (RW) уместнее
   Приоритет: проверить `===` рёбра первыми (самая частая ошибка).
