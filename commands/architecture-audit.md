...
### 6.3 — Recurrence after fix

Категория с **≥ 3 addressed** И **≥ 2 open** → ⚠️ critical blind spot: фиксы не работают структурно, паттерн возвращается. Самый сильный сигнал необходимости Level 4+ решения.
...
## ❓ Требует решения PM
- {strategic decision требующий human approve}
...
⚠️ /architecture-audit **никогда** не merge сам. Только PR draft → `/plan` → `/code` → `/review` → `/deploy` для каждого структурного предложения.
