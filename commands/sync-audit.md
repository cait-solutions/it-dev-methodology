...
**Если `auto_pull: true` в `CLAUDE.local.md ## Auto-update`:** автоматически выполнить вариант (a) без вопроса. При ошибке → показать инструкцию варианта (b), не падать молча. Показать только результат.
