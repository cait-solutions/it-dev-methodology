...
**Если выбрано (a):** выполнить pull. При ошибке (non-ff) → сообщить пользователю о diverged branches. При auth-ошибке → показать инструкцию варианта (b).
**Если выбрано (b):** показать точные команды и ждать подтверждения "готово".
...
**После успешного pull (варианты a и b) — auto-apply (self-heal, без вопроса, closes G-092):**
```bash
bash scripts/sync-methodology.sh .
```
- ✅ OK → сообщить: `✅ Методология обновлена и применена (commands/, hooks/ актуальны). Перезапусти сессию чтобы новые команды вступили в силу.`
...
**Если `auto_pull: true` в `CLAUDE.local.md ## Auto-update`:** автоматически выполнить вариант (a) + sync-apply без вопроса. При pull-ошибке → показать инструкцию варианта (b), не падать молча. Показать только результат.
