...
6. Вывести список найденных `[no-marker]` repos и для каждого задать вопрос (по одному):
   ...
   ```
7. **При `init` — ⛔ pre-init pre-flight (G-118: distributed-state check ОБЯЗАТЕЛЕН):**

   Прежде чем запустить `new-project-init.sh` — убедиться что репо действительно пустое во всех клонах, не только локально. `git ls-files` / `find` видят только **локальный** диск; другой клон с незапушенными артефактами невидим → init поверх него создаёт дивергенцию и merge-конфликты при последующем push.

   **7a. Git fetch + remote branches check:**
   ```bash
   git -C "<abs_path>" fetch origin 2>/dev/null
   REMOTE_BRANCHES=$(git -C "<abs_path>" branch -r 2>/dev/null)
   ```
   - Если `fetch` упал (network/auth) → показать:
     ```
     ⚠️ pre-init: не удалось проверить remote-состояние <repo-name> (fetch failed).
        Init заблокирован — не смог подтвердить что репо пустое на remote.
        Проверь вручную: git -C <abs_path> fetch origin && git -C <abs_path> branch -r
        Если remote пустой — ответь на вопрос ниже вручную.
     ```
     → задать вопрос 7b вручную без fetch-данных.
   - Если remote ветки **содержат** коммиты с `.claude/` или `docs/` → показать:
     ```
     ⚠️ pre-init: <repo-name> уже имеет инициализированные ветки на remote.
        ...
        Рекомендованный путь: получи эти ветки → git pull → затем запусти /sync-audit как [drift].
        Init поверх существующего remote создаст дивергенцию.
     ```
     → предложить `pull-first` вместо init: `git -C "<abs_path>" pull origin <branch>` → перейти к sync как `[drift]`.

   **7b. Явный вопрос про другой клон:**
   ```
   ❓ pre-init: <repo-name> — нет инициализации локально.
      ...
      да   — в другом клоне есть работа → сначала запушь её, затем повтори /sync-audit
      пропустить — отложить init (статус [not-initialized] останется)

   Ответ (нет / да / пропустить):
   ```
   - `да` → ⛔ **init ЗАПРЕЩЁН**: «Сначала запусти `git push` из клона с незапушенными файлами — это сохранит твои данные. После push повтори /sync-audit, репо появится как [drift] и будет синкнут без потерь.» → пропустить init.
   - `пропустить` → перейти к Шагу 8 (skip).
   - `нет` → продолжить к 7c.

   **7c. Стандартные pre-checks и запуск:**
...
4. При выборе **init** — ⛔ pre-init pre-flight (G-118: distributed-state check):

   LAR-init пишет файл в репо консьюмера. Перед записью — проверить что нет незапушенного состояния в другом клоне (тот же паттерн что Gap 14 п.7).

   **4a. Git fetch + вопрос про другой клон:**
   ```bash
   git -C "<doc_repo_path>" fetch origin 2>/dev/null
   ```
   - fetch fail → предупредить (не смог проверить remote) → задать вопрос вручную.
   - remote содержит `docs/architecture/LIVING-ARTIFACTS.md` → предложить `git pull` вместо init.

   ```
   ❓ pre-init (LAR): <repo-name> — LIVING-ARTIFACTS.md отсутствует локально.
      Есть ли другой клон этого репо с незапушенными файлами (включая LAR)?

      нет  — продолжить init LAR
      да   — сначала запушь из другого клона, затем повтори /sync-audit
      пропустить — отложить (Gap 16 спросит снова)

   Ответ (нет / да / пропустить):
   ```
   - `да` → ⛔ **init ЗАПРЕЩЁН**: направить на push-first. → пропустить.
   - `пропустить` → skip.
   - `нет` → продолжить к 4b.
