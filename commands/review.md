...
- **Normative-semantics adversarial gate (closes G-133 class):** `git diff main..HEAD --name-only` трогает `commands/*.md` / `commands-local/*.md` / `CLAUDE.md`? → прочитать `last_plan_session.adversarial_check`. Если `ran != true` ИЛИ поле отсутствует (старый план без поля) → 🔵 Recommendation: «diff меняет нормативный контент без recorded adversarial check — запусти `/plan` с этим PR scope ИЛИ вручную запусти critic. Disposition: ran (уже прогонялся) / typo-irrelevant (чисто косметика, семантика не менялась) / should-rerun (семантика изменилась)». NB: `last_plan_session` может быть stale (другой план) — disposition `typo-irrelevant` принимается без проверки (не блок).
