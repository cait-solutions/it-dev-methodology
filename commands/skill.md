...
| `--scope portfolio` | Skill предназначен для агрегации через `/pull-consumers` Ось 7 Layer 2 (дефолт: `local`) |
...
Если `portfolio`:
```
   Будет агрегирован через /pull-consumers при Ось 7 Layer 2 (≥3 проектов с North Star).
```
```
