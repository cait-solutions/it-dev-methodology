...
- [ ] **Показ обновлённых карт пользователю** *(контроль процесса):* после `update-mermaid-links.sh` — вывести список map-артефактов которые этот /code реально обновил, с их свежими mermaid.live ссылками **и текстовым описанием что изменилось** в каждой карте:
  ```
  ...
  ```
  - **Текстовый diff** под каждой ссылкой — это замена визуальной подсветки в самой карте (style-строки раздували бы URL > 2000 и требовали хранить прошлое состояние). Формат: `+ добавлено`, `~ изменено`, `− удалено`; ноды и связи. Перечислять **все** изменения карты этим /code, не сокращать.
  ...
  - URL берётся из обновлённой ссылки в файле (та что только что записал `update-mermaid-links.sh`); для URL > 2000 — указать "(скопировать из файла, > 2000 символов для клика)".
