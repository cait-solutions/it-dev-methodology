...
- [ ] **ROADMAP Done-trigger (PR-coupling, closes G-101):** если план закрывает milestone или feature-axis (работа из ROADMAP.md ## Now секции завершена) → переместить соответствующую запись в `## Done` таблицу в этом же /code. Критерий: основная часть milestone реализована и задеплоена — не 100% (edge cases OK deferred). Пропустить при: typo/bugfix/refactor не являющийся самостоятельным milestone. ROADMAP.md отсутствует → graceful skip.
