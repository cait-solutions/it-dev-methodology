...
**Pre-flight model check:** **да — при старте команды** спроси пользователя какая модель активна (или используй ранее подтверждённую в сессии) и сравни с tier из `/plan` рекомендации. При mismatch — пауза + рекомендация (порог и формат сообщения: `.claude/model-tiers.md § Pre-flight model check`; under-powered ловится на любой ступени вниз → громкий STOP-advisory).

---

## Режим (наследуется из /plan)

**Режим Lite/Full** — по канону `.claude/task-types.md`. Кратко: Lite = локальный фикс
≤ 1 файла < 20 строк (тогда сокращённый набор точек ниже); Full = всё остальное.
...
4. **Anti-cheat (no-gate-weakening) — closes no-gate-weakening class:** ⛔ Никогда не ослабляй **артефакт** или **критерий**, чтобы пройти квалити-гейт. Удовлетвори гейт по существу — не меняй сам измеритель.
   - **Универсально (любой гейт):** `/review`, `/doc-audit`, `validate-maps-coverage`, acceptance-критерий из `/plan`, тест — все попадают под правило.
   - **Домен разработки (пример):** не отключай / не удаляй падающий тест · не правь реализацию только чтобы check позеленел · не over-mock'ай чтобы обойти coverage.
   - **Не-dev домены (пример):** не удаляй обязательную секцию артефакта чтобы пройти `/doc-audit` · не ослабляй acceptance-критерий чтобы пройти `/review` · не выкидывай узел карты чтобы пройти `validate-maps-coverage`.
   - **Допустимо** (это НЕ cheating): осознанно изменить гейт/критерий **как явное решение** с обоснованием в плане/DEVLOG (например, тест был неверным — исправить тест + записать почему). Граница: меняешь измеритель ради прохождения = cheat; меняешь измеритель потому что он был неправильным + named обоснование = легитимное решение.
5. Если причина в другом сервисе → СТОП, OPEN-QUESTIONS.md
...
9.5. **Maps coverage surfacing (closes BS-1 / P-009 — content-актуальность СРАЗУ после /code, не только на deploy):** после финализации карт — запустить **non-blocking** coverage-report. Раньше `validate-maps-coverage.sh` (gate, exit 1) жил **только** в `deploy-push.sh` → coverage-drift (команда/skill/компонент отсутствует в карте) был невидим до деплоя. Этот surfacing показывает drift в момент /code.
    ```bash
    bash scripts/validate-maps-coverage.sh --report
    ```
    - `[WARN]` / `[ERROR]` строки (команда/skill/компонент отсутствует в USER-MAP/ARTIFACT-MAP/SYSTEM-MAP, или диаграмма stale по `diagram-sources`) → **показать пользователю** + добавить недостающие строки карт в этом же PR.
    - Чисто (0 warn / 0 err) → ✅ продолжить.
    - **Non-blocking** (`--report` всегда exit 0): не блокирует /code — это раннее surfacing. Жёсткий gate остаётся на `deploy-push.sh` (последний рубеж). `/review` повторяет surfacing перед merge.
    - Скрипт отсутствует (consumer на старой версии) → graceful skip: `⚠️ validate-maps-coverage.sh не найден — sync методологию до v5.47.0+`.
    - **Semantic PR-couple (BS-2/BS-5, ADR-015 слой 1 — semantics, не presence):** `--report` ловит **presence** (узел/строка есть в карте). НЕ ловит **семантику** (стрелка отражает реальную связь, label «Зачем» ещё верен). Поэтому здесь же — ручная сверка по diff этого /code:
      - Для каждого **компонента, чей файл затронут в diff** (новый/изменённый/удалённый/переименованный): `grep` его имя/ID в живых картах (SYSTEM/USER/ARTIFACT-MAP, ROADMAP).
      - Узел найден → подтвердить что его **связи (стрелки) и label** ещё соответствуют тому что изменил diff. Изменилось (связь добавилась/исчезла, назначение сместилось, компонент удалён/переименован) → **обновить карту в этом же PR** (PR-coupling).
      - Узла нет, но компонент достаточно крупный для карты → это coverage-gap (его поймает `--report` выше).
      - Diff не трогает компоненты с узлами в картах (чистый рефакторинг/typo/внутренняя логика) → строка `semantic-couple: N/A — нет затронутых узлов карт`.
      - ⛔ Это L3 (procedural prompt), не 100%-гарантия — periodic safety-net = `/architecture-audit` Способность D (ADR-015 слой 2). Цель: не дать семантике устареть молча между аудитами.
...
- [ ] **CHANGELOG.md rule:** если PR добавляет новый consumer-facing feature (новый скрипт в templates/scripts/, новая команда, новое правило поведения агента, изменение формата) — добавить запись в `CHANGELOG.md` сверху с: version, title, что добавилось, actions (ordered bash commands), priority (🔴/🟡/🟢). Без этого `/push-consumers` drift-таблица не покажет новый feature консьюмерам.
...
- [ ] **CLAUDE.md WHAT/WHY split-coupling (closes claude-md-bloat-recurrence class, [opinion:claude-md-split-mechanism] council 7/7 2026-06-30):** добавляя/расширяя правило в CLAUDE.md — в сам CLAUDE.md идёт **только WHAT** (норма + где enforcement, ≤~5 строк прозы на правило). Весь WHY (rationale, примеры, edge-cases, история, разбор `closes G-NNN`) → парным блоком в **CLAUDE_LONG.md** под тем же якорем-заголовком. ⛔ Запрещено писать абзац-rationale внутри CLAUDE.md. Проверка перед commit: новая/изменённая секция CLAUDE.md ≤ ~5 строк прозы? Нет → перенести излишек в CLAUDE_LONG.md (cross-ref вместо копии). Бьёт **в момент создания правила** (где раздувание и рождается), не gate — блокирующий size-gate отклонён council'ом (anti-cheat: легчайший зелёный путь = удалить правило; Ось 1/5: «gaming неотличим машиной»). `validate-artifact-size.sh` (п.10 выше) остаётся **confirmer**, не primary. Эскалация к by-construction-рендеру (CLAUDE.md генерируется из what/why-источника, L2) — только по evidence рецидива (RISKS R-09).
- [ ] **Design Spec update (PR-coupling):** если план затрагивает фичу покрытую Design Spec (`docs/services/*/` или `docs/architecture/*_DESIGN.md`) → обновить Design Spec в этом же PR. Проверка актуальности: `git log -1 --format=%ad <design-spec-file>` vs `git log -1 --format=%ad <code-file>` — код новее → STALE → обновить. Пропустить при: рефакторинг без поведенческих изменений, performance fix, typo. Нет Design Spec для фичи → graceful skip.
- [ ] **Новый validator PR-coupling (closes G-112 class):** если PR добавляет `scripts/validate-*.sh` → **обязан** добавить в том же PR: negative-fixture в `scripts/fixtures/validators/` + `assert_exit` строку в `scripts/test-validators.sh` registry + строку в `scripts/fixtures/validators/README.md`. Dual-copy (ADR-014): фикстур также в `templates/scripts/fixtures/validators/`. Без negative-fixture proof-of-rejection отсутствует → `/review` 🔴 блокирует merge.
- [ ] **LIVING-ARTIFACTS.md update (PR-coupling):** если план создаёт/изменяет mechanism (hook / script / command / map / config / registry / template) → обновить `docs/architecture/LIVING-ARTIFACTS.md` (или consumer-эквивалент) в этом же PR. Конкретно: добавить строку для нового артефакта из /plan Шаг 97 Sustainment Declaration, или обновить строку если lifecycle изменился. LIVING-ARTIFACTS.md отсутствует → создать из `templates/LIVING-ARTIFACTS.template.md`, затем добавить строку. Пропустить при: typo/refactoring без изменения lifecycle, Lite mode изменение без нового механизма.
- [ ] **ROADMAP Done-trigger (PR-coupling, closes G-101 + P-008):** если план закрывает methodology milestone — добавить запись в `## Done` в этом же /code:
  ...
  - Критерий «milestone»: задача имеет самостоятельный task_id И закрывает gap или добавляет capability методологии. Критерий «завершён»: основная часть реализована и задеплоена — edge cases OK deferred.
  - Пропустить при: typo/bugfix/refactor не являющийся самостоятельным milestone. ROADMAP.md отсутствует → graceful skip.
...
- [ ] **Draft maps cleanup:** если `/plan` создавал `_tmp_draft-maps-*.md` — удалить **после** финализации living maps:
  ```bash
  rm -f _tmp_draft-maps-*.md
  ```
  ⛔ Удалять **строго после** обновления и commit living maps — наличие `_tmp_draft-maps-*` = диагностический сигнал «PR-coupling не закрыт». Порядок: (1) обновить living maps → (2) commit карт → (3) `rm _tmp_draft-maps-*.md`. Остались файлы после /code → карты не финализированы.
  - Ad-hoc «было→станет» (вариант 4 `/plan` Шаг 99.54): в living map не финализируется → удалять сразу.
  - Не создавался в этом `/plan` → `ls _tmp_draft-maps-*.md 2>/dev/null` = empty → graceful skip.
...
**⛔ Visual-parity pre-fix protocol (ДО первого Edit — closes G-089 axes + visual-parity completeness класс):**

Если visual-задача про **выравнивание / эквивалентность** (`«сделай как»`, `«выровняй»`, `«должны быть одинаковые»`, `«почему X толще/больше Y»`, `«привести к стандарту»`) И затрагивает **≥2 элемента/surface** — **ОБЯЗАТЕЛЕН полный pre-fix замер ДО первого фикса**. Не чинить «по одному различию за итерацию».

**Почему (механизм G-089 + эволюция пропусков):** визуальное «толще/крупнее/иначе» — multi-source (несколько разных причин). Агент хватает первое правдоподобное различие, фиксит, верифицирует «выглядит ок», коммитит → пользователь видит «не изменилось» → следующее различие → следующее. Частичные раунды вместо одного. Каждый раунд закрывает **только то что попало в текущий скриншот** — пропуск всплывает на следующем слое (одна ось → все оси → под-элемент → источник разметки). Post-fix DOM verification (ниже) ловит «отрендерилось ли», но НЕ «сравнил ли я с целью полно».

Полнота требует **трёх** измерений — пропуск любого = частичный раунд:

**(1) Эталон-как-артефакт — сравнивай с зафиксированным, не с памятью.**
Прежде чем приводить что-либо «к стандарту/эталону» — **зафиксируй эталон в артефакт** (замеренные значения целевого вида), сравнивай с ним. «Эталон в голове / в скриншоте» каждый раз восстанавливается неполно → измеряешь заново и упускаешь. Формат и место артефакта — на усмотрение проекта (значения рядом со стилями / в doc / inline в задаче); методология требует лишь **наличие зафиксированного эталона**, а не повторное «припоминание» цели.

**(2) Полный чеклист под-элементов surface — не «что видно в скриншоте».**
Инвентаризация каждого surface обязана пройти **фиксированный набор под-элементов** (не только бросающиеся в глаза):
`{заголовок · строки/элементы списка · поля ввода · поиск · футер/пагинация · кнопки-действия · скроллбар · границы/разделители}`
Матрица для замера = **surface × под-элемент × ось**. Пропущенный под-элемент (частая жертва — пагинация, скроллбар, разделители) = частичный паритет, всплывёт следующим раундом.

**(3) Component-source check — один ли это генератор разметки.**
Перед «привести surface X к surface Y» — проверь: **производит ли их один и тот же компонент-источник**? Если X и Y рендерятся **разными** компонентами (разный генератор разметки → разная структура) — паритет **только через стили недостижим**: стилевое правило для одного не переносится на другой. Нужно явное решение: унифицировать компонент-источник ИЛИ применить **per-source** правило с признанием «это два разных вывода разметки». Молчаливое «дам общий стиль» на разных источниках = скрытый частичный фикс.

**Один проход — таблица всех осей у всех элементов** (оси — стандартные CSS-свойства, замер через runtime computed style):
```js
// Замерить КАЖДЫЙ элемент по КАЖДОЙ оси за один проход (getComputedStyle / эквивалент стека):
const AXES = ['fontSize','fontWeight','letterSpacing','lineHeight','color','backgroundColor',
              'textTransform','height','paddingLeft','paddingTop','borderWidth'];
// → таблица element × axis → найти ВСЕ расхождения разом
```

| Элемент | font-size | weight | letter-spacing | color | background | height |
|---|---|---|---|---|---|---|
| Эталон (целевой вид) | 13px | 500 | normal | #d4d4d4 | transparent | 34px |
| Элемент A | 13px | 400 | 1.16px | #d4d4d4 | #262626 | 42px |
| **Δ расхождения** | — | **400 vs 500** | **1.16 vs normal** | — | **#262626 vs transparent** | **42 vs 34** |

Найти **все** строки `Δ` по **всем** под-элементам **всех** surface → **один** фикс-набор закрывает все расхождения. Не коммитить пока не закрыты все выявленные.

⛔ **Первый Edit visual-alignment задачи без (1) зафиксированного эталона + (2) полной матрицы surface×под-элемент×ось + (3) component-source check = шаг не завершён** (тот же gate что DOM verification ниже). Single-element правка (один элемент, нет «выровнять относительно другого») — pre-fix протокол не обязателен, сразу к путям верификации.

> **NB (reactive backstop):** если pre-fix протокол всё же пропущен и началось iteration-залипание — `iteration-watchdog.py` (ступень-1 на N=3) напомнит «pattern-comparison + DOM-измерение». Pre-fix протокол здесь — **превентивный** L3-слой (не ждать N итераций), watchdog — реактивный L4. При `reset_on_commit: true` (default) watchdog слеп к commit-per-iteration — поэтому prevention важнее (см. CLAUDE.local.md `## Iteration watchdog` для frontend-heavy проектов).

**Три допустимых пути верификации (выбрать один) — ПОСЛЕ фикса:**
...
**💡 Capture-nudge (in-session re-discovery, closes P-001):** прежде чем закрыть шаг — оцени **один** вопрос: *«переоткрыл ли я в этой сессии факт, который уже был известен ранее — рабочую команду, устройство проекта, verified path, ограничение — и потратил на это время?»* Если ДА (ты выводил заново то, что уже было решено) → одна строка пользователю:
```
💡 В этой сессии я переоткрыл: <что именно>. Зафиксировать в /skill (домен «<домен>»), чтобы следующая сессия не повторяла? (y/skip)
```
- **НЕблокирующий**, opt-out (`skip` — молча продолжить). Не спрашивать если ничего не переоткрывал.
- ⚠️ Это **комплемент** структурному детектору `auto-update-watchdog.py` (SessionStart): хук ловит **накопленный** сигнал (≥3 находки домена без skill, из DEVLOG) на старте; этот nudge ловит **in-session** переоткрытие, которое хук посчитать не может (ещё не в DEVLOG). Оба ведут к `/skill`.
- Механика (regulator level): хук = L3 структурный (счётный сигнал); этот nudge = L6 prompt (self-assessment) — намеренно, т.к. «переоткрыл ли я факт» машина не детектит. НЕ дублировать хук здесь: если сигнал уже накоплен в DEVLOG — его покажет хук в следующей сессии, не этот nudge.
