...
**⛔ Pre-fix baseline measurement (ДО первого Edit — closes G-089):**

Если visual-задача про **выравнивание / эквивалентность** (`«сделай как»`, `«выровняй»`, `«должны быть одинаковые»`, `«почему X толще/больше Y»`) И затрагивает **≥2 элемента** — **ОБЯЗАТЕЛЕН один runtime-замер ВСЕХ осей различия у ВСЕХ элементов ДО первого фикса**. Не чинить «по одной оси за итерацию».

**Почему (механизм G-089):** визуальное «толще/крупнее» — multi-source (несколько разных причин у разных компонентов). Агент хватает первую правдоподобную ось (`letter-spacing`), фиксит, верифицирует «выглядит ок», коммитит → пользователь видит «не изменилось» → следующая ось (`font-weight`) → следующая (`background`). 3-4 итерации с коммитами вместо одной. Post-fix DOM verification (ниже) ловит «отрендерилось ли», но НЕ «сравнил ли я с целью по всем осям».

**Один проход — таблица всех осей у всех элементов:**
```js
// Playwright / getComputedStyle — замерить КАЖДЫЙ элемент по КАЖДОЙ оси за один проход:
const AXES = ['fontSize','fontWeight','letterSpacing','color','backgroundColor',
              'textTransform','height','paddingLeft','paddingTop'];
// → вывести таблицу element × axis → найти ВСЕ расхождения разом
```

| Элемент | font-size | weight | letter-spacing | color | background | height |
|---|---|---|---|---|---|---|
| Кнопка A | 13px | 500 | 1.16px | … | transparent | 34px |
| Кнопка B | 13px | 400 | normal | … | #262626 | 34px |
| **Δ расхождения** | — | **400 vs 500** | **1.16 vs normal** | — | **transparent vs #262626** | — |

Найти **все** строки `Δ` → **один** фикс закрывает все оси сразу. Не коммитить пока не закрыты все выявленные расхождения.

⛔ **Первый Edit visual-alignment задачи без таблицы замеров всех осей = шаг не завершён** (тот же gate что DOM verification ниже). Single-element визуальная правка (один элемент, нет «выровнять относительно другого») — pre-fix замер не обязателен, сразу к путям верификации.

> **NB (reactive backstop):** если pre-fix замер всё же пропущен и началось iteration-залипание — `iteration-watchdog.py` (ступень-1 на N=3) напомнит «pattern-comparison + DOM-измерение». Pre-fix правило здесь — **превентивный** L3-слой (не ждать N итераций), watchdog — реактивный L4. При `reset_on_commit: true` (default) watchdog слеп к commit-per-iteration — поэтому prevention важнее (см. CLAUDE.local.md `## Iteration watchdog` для frontend-heavy проектов).
