...
11. **[methodology] Формат артефакта изменён** (новый формат ссылок, секций, placeholder'ов в картах или командах): проверить соответствующие `templates/*.template.md` — применён ли тот же формат?
    ```bash
    grep -r "<старый паттерн>" templates/
    ```
    Если найдены несоответствия — исправить шаблон в **этом же PR**. *(closes G-068)*
