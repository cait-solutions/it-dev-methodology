...
> **Цель:** реализовать утверждённый /plan атомарными commits + обновить документацию (PRODUCT.md, карты, ADR) в том же PR. /code НЕ генерирует план — он его исполняет. Branch check + self-review 8 точек + финализация Mermaid карт + sync validators в /review.
