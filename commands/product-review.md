...
**ROADMAP Mermaid sync (PR-coupling):** если в этом `/product-review` перенесены записи в ROADMAP.md (добавлены/изменены/закрыты узлы) — обновить секцию `## Визуальный roadmap` в ROADMAP.md: отредактировать mermaid-блок, затем `bash scripts/update-mermaid-links.sh <roadmap-path>`. Если mermaid-блока ещё нет — добавить по шаблону из `templates/ROADMAP.template.md`. Только textual редакция секций без изменения нодов — не обновлять.
