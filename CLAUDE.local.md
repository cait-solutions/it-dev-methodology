...
**Исключение — `/push-consumers` Шаг 3 init write-only:** создаёт файлы через `new-project-init.sh` для `[not-initialized]` репо после явного per-repo подтверждения (init / skip / never). Git commit остаётся за пользователем.
...
exclude_paths:                  # абсолютные пути — /push-consumers "never" (Шаг 3) пишет сюда
  - "l:/Projects/AI assistant folder/AI assistant"  # code-repo, not documentation — never (2026-06-23)
                                # /pull-consumers и /push-consumers молча пропускают эти репо
```
...
Конфиг для `auto-update-watchdog.py` SessionStart hook (read-only детектор, push-only consolidation).
...
`methodology_path: .` — methodology-platform это self (this repo IS the methodology). Hook **только детектирует** version-drift и hook-health (read-only) — НЕ запускает sync, НЕ пишет .claude/. Доставка обновлений консьюмерам — push-only через `/push-consumers`; self-apply методологии — через `deploy-push.sh` после merge. *(Удалены `interval_hours`/`on_failure`/`audit_threshold`/`auto_pull` — относились к removed UPDATE-mode self-sync.)*
