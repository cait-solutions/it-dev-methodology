...
while IFS= read -r repo_path; do
  ...
  # Fetch ВСЕ ветки (не только agent_branch) — нужно для branch-audit ниже.
  # Pull/safe-reset ниже трогает ТОЛЬКО agent_branch (by-design): fetch-scope шире
  # pull-scope намеренно — audit показывает divergence других веток (напр. main),
  # но автоматически их не двигает (риск перетереть намеренно отстающую ветку).
  fetch_out=$(git -C "$repo_path" fetch origin 2>&1 | _sanitize || true)
  fetch_exit=$?
  ...
  while IFS=' ' read -r local_ref upstream_ref; do
    ...
    if [[ "${ahead:-0}" -gt 0 || "${behind:-0}" -gt 0 ]]; then
      marker=""
      [[ "$local_ref" == "$branch" ]] && marker=" (agent-ветка — pull ниже)"
...
if [[ $DIVERGENT_BRANCHES -gt 0 ]]; then
  ...
  echo "      Только agent-ветка пуллится автоматически. Другие — реши вручную:"
  echo "      git checkout <ветка> && git pull --ff-only  (или /pull --current на ней)."
fi
if [[ $ERRORS -gt 0 ]]; then
  echo ""
