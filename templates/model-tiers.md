...
| `/ux-audit` | **Default** | High | ON | флоу пересекает 3+ сервиса/компонента; ≥5 friction-points одновременно (синтез приоритизации) | ❌ Не downgrade to Fast — оценка friction требует сравнения с эталоном, не механический прогон |
...
| `/last-repo-changes` | **Default** | Medium | ON | (никогда — объём ограничен дампом коллектора) | Consumer-facing. Read-only синтез: что подтянулось/на remote/divergence по всем repos workspace, простым языком, причины из DEVLOG-diff. НЕ git-механика (в отличие от /pull) — синтез требует reasoning, Fast пропустит группировку по смыслу. Запускает repo-changes-collect.sh |
