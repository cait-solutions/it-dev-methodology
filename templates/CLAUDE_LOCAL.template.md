...
- `reset_on_commit` — обнулять ли счётчик файла при commit. `true` (default) = да, RPN-150-safe между задачами. `false` = счётчик переживает коммиты в пределах сессии — **ловит commit-per-iteration reasoning-залипание** (агент коммитит после каждого фикса одного бага → при `true` ступень-1 недостижима). Trade-off: при `false` счётчик может врать между разными задачами на одном файле.
  > **Рекомендация для frontend-heavy проектов (закрывает G-089-класс):** если в проекте частые visual-итерации с commit-per-fix (CSS/Vue с коммитом после каждой правки) — поставь `reset_on_commit: false`. При `true` watchdog **слеп** к commit-per-iteration thrash (самый частый visual failure-mode): каждый commit обнуляет счётчик, ступень-1 (N=3) недостижима. Превентивный слой — `/code` pre-fix baseline measurement (измерь все оси одним проходом до первого фикса) — работает независимо от этого флага, но `false` восстанавливает и reactive backstop. Default остаётся `true` (безопасен для non-frontend проектов где правки одного файла редки).
