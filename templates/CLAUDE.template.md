...
**Plain-language output rule:** команда, выдающая пользователю **аналитический вывод** (синтез / вердикт / рекомендации / findings), **завершает вывод коротким резюме простым языком** — блок `## Простыми словами` (2-5 строк): что это значит и что делать дальше, понятно без расшифровки жаргона. **Резюме завершается конкретной committed-рекомендацией / следующим шагом** («Рекомендую X» / «Предлагаю Y» / «Следующий шаг — Z») — как `/opinion` Committed Verdict коммитит позицию; открытый вопрос допустим только ПОСЛЕ явной рекомендации, не вместо неё. Резюме, заканчивающееся только вопросом («сделать A или B?») без названной рекомендации — **не зачтено**. Остальной вывод (метки, разбор, технические детали) **остаётся как есть** — резюме добавляется в конце, ничего не переписывается и не прячется. Цель: получатель понимает суть и видит что предлагается без декодирования. Применяется к аналитическим командам: `/opinion`, `/plan`, `/research`, `/retro`, `/architecture-audit`, `/diagnose`, `/review`, `/roadmap`, `/product-check`, `/vision`, `/scan-sources`, `/scan-sources-full`, `/scope-out`, `/doc-audit`, `/marketing`, `/last-repo-changes`, `/pull-consumers`, `/push-consumers`. **Не** применяется к механическим/state-командам (`/pull`, `/deploy` чеклист, `/code`, `/secrets`, `/push-merge`, `/push-only`, `/test`, `/onboard`, `/skill`) и к **внутренним артефактам** (DEVLOG-теги, verdict в журнале, AGENT-GAPS — там жаргон уместен). **Новая аналитическая команда наследует это правило** — её output-секция обязана заканчиваться блоком-резюме простым языком (как Model tier rule требует `## Рекомендуемая модель`). Обобщает существующие прецеденты: `/scan-sources` Шаг 2.6, `/plan` Confidence Declaration, `/opinion` Committed Verdict. Это L6 prompt-правило для presentation-слоя (валидатор «простоты» не строим — машина не отличит; over-engineering). Enforcement = always-loaded правило + inline блок в output-секции команды (читается при исполнении); review-гейт на формат отвергнут как downstream. Само правило и резюме пишутся просто — это критерий приёмки.
...
- **URL над блоком: bare URL only** — никаких `[текст](url)` обёрток. После правки mermaid-блока агент запускает `bash scripts/update-mermaid-links.sh <file>` (hook — страховка, не primary). Если скрипт не найден — сообщить явно.
