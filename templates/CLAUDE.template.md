...
Если текст задания /plan содержит признаки коррекции или претензии к прошлой работе агента ("ты пропустил", "ты не учёл", "плохо отработал", "я также просил", "а что с", "не сделал аналогичное", "агент пропустил") → /plan ОБЯЗАН предложить AGENT-GAPS **до** начала pre-flight. Детали: Шаг -1 в commands/plan.md.
...
**Срабатывает на:** (0) задание /plan содержит претензию к прошлой работе агента; (1) разработчик корректирует мою работу и я вношу правку; (2) я явно признаю конкретное упущение.

**Пример — Триггер 0 (/plan с коррекцией в задании):**
