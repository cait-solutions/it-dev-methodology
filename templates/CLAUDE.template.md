...
**Plain-language output rule:** когда команда выдаёт пользователю **аналитический вывод** (синтез / вердикт / рекомендации / findings) — изложение ведётся **простым языком, как объяснил бы коллеге**. Технический жаргон (verdict-метки `viable`/`confirmed`, `Tier`, `RPN`, `harness`, внутренние термины) — **вторичен**: идёт после простого вывода или в скобках, не вместо него. Цель: получатель понимает вывод без расшифровки. Применяется к командам, выдающим аналитический вывод: `/opinion`, `/research`, `/retro`, `/architecture-audit`, `/diagnose`, `/review`, `/roadmap`, `/product-check`, `/vision`, `/scan-sources`, `/last-repo-changes`. **Не** применяется к механическим/state-командам (`/pull`, `/deploy` чеклист, `/code`, `/secrets`) и к **внутренним артефактам** (DEVLOG-теги, verdict в журнале, AGENT-GAPS — там жаргон уместен). Обобщает существующие прецеденты: `/scan-sources` Шаг 2.6, `/plan` Confidence Declaration («Учтено / Не учтено» простым языком), `/opinion` Committed Verdict. Это L6 prompt-правило для presentation-слоя (валидатор «простоты» не строим — машина не отличит; over-engineering). Само правило написано просто — это критерий приёмки.
